...
Run: `antfarm workflow install <name>`
...
## Antfarm Dispatch Rule (MANDATORY)

For ALL code/feature/build/refactor tasks:

1. NEVER self-implement — always dispatch
2. Run: bash agent-system/dispatch.sh "<task>" code
3. Wait for Antfarm STATUS: done
4. Review output, report back

## Skill Pre-Flight Rule (MANDATORY)

Before ANY skill execution:

1. Read full SKILL.md spec
2. Map exact required dir/file structure
3. Confirm write path is /data/ not /root/
4. Execute completely — no partial work
5. Verify output matches spec before reporting done

## Memory (BrainX — Every Session)

1. Read `SOUL.md`
2. Read `USER.md`
3. Read `AGENT_OS_CONTEXT.md`
4. Read `BRAINX_CONTEXT.md` ← auto-injected from Postgres on every bootstrap
5. Read `.learnings/ERRORS.md`
6. Read `.learnings/LEARNINGS.md`
